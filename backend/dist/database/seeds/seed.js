...
async function seedDatabase(dataSource) {
    ...
    if (worker && (shouldCreateBaseSamples || ticketCount < targetTotal)) {
        const sampleTickets = shouldCreateBaseSamples
            ? [
                {
        ...
        const workers = await userRepository.find({
            where: { role: user_entity_1.UserRole.WORKER, isActive: true },
        });
        const statusCycle = [
            ticket_entity_1.TicketStatus.OPEN,
            ticket_entity_1.TicketStatus.IN_REVIEW,
            ticket_entity_1.TicketStatus.IN_PROGRESS,
            ticket_entity_1.TicketStatus.SOLVED,
        ...
        const categoryCycle = [
            ticket_entity_1.TicketCategory.SOFTWARE,
            ticket_entity_1.TicketCategory.HARDWARE,
            ticket_entity_1.TicketCategory.MECHANICAL,
            ticket_entity_1.TicketCategory.ELECTRICAL,
            ticket_entity_1.TicketCategory.PLUMBING,
            ticket_entity_1.TicketCategory.TASK,
        ];
        const extraTickets = [];
