...
export const TICKET_INQUIRY_MARKER_RE =
  /\[TICKET_INQUIRY:(await_query|found)\]/;

export type TicketInquiryAspect =
  | 'overview'
  | 'description'
  | 'status'
  | 'assignment'
...
export function tagInquiryReply(kind: 'await_query' | 'found', text: string): string {
  return `[TICKET_INQUIRY:${kind}]\n${text}`;
}

export function stripInquiryMarker(text: string): string {
...
function isLookupPromptMessage(content: string): boolean {
  if (/\[TICKET_INQUIRY:await_query\]/.test(content)) return true;
  const c = stripInquiryMarker(content);
  return /which ticket should i look up|quel ticket voulez-vous consulter/i.test(c);
}

/** Techo asked which ticket — and the user has not answered yet. */
export function isAwaitingTicketLookupQuery(
  history?: { role: string; content: string }[],
): boolean {
...
/** User recently said they want info on an existing ticket (not create). */
export function hasRecentTicketInquiryContext(
  history?: { role: string; content: string }[],
): boolean {
  if (!history?.length) return false;
  if (isAwaitingTicketLookupQuery(history)) return true;
  const userBlob = history
    ...
    .toLowerCase();
  return (
    /\b(ask|question|info|check|look up|lookup|find|search|tell me|show me|about|know|consulter)\b.*\btickets?\b/.test(
      userBlob,
    ) ||
...
export function isTicketInquiryFollowUp(message: string): boolean {
  const m = message.trim().toLowerCase();
  if (!m) return false;
  if (/\b(create|open|make|créer)\s+(a\s+)?(new\s+)?ticket\b/.test(m)) return false;
  // Close/update/delete → action handler, not read-only lookup loop
  if (isTicketActionIntent(message)) return false;
  return (
    /\b(it|this|that|the ticket|ce ticket|same one)\b/.test(m) ||
    (m.length <= 100 &&
      /\b(open|closed|assign|assigned|status|priority|description|problem|who|machine|area|still|yet)\b/.test(
        m,
      ))
  );
}

/** Last user message that looks like a ticket title/search term during inquiry. */
export function findRecentTicketSearchTermFromHistory(
  history?: { role: string; content: string }[],
): string | null {
  if (!history?.length) return null;
...
export function extractBareSearchQuery(message: string): string | null {
  const bare = message.trim().replace(/^["']|["']$/g, '');
  if (bare.length < 3 || bare.length > 200) return null;
  if (isConversationEndUserMessage(bare)) return null;
  if (/\b(create|make|open|créer)\s+(a\s+)?ticket\b/i.test(bare)) return null;
  if (/^(hello|hi|hey|bonjour|thanks|merci)\b/i.test(bare)) return null;
  return bare;
}

/** Should the ticket lookup handler run (not the create wizard). */
export function shouldProcessTicketInquiry(
  message: string,
  history?: { role: string; content: string }[],
  ...
  if (isTicketInquiryIntent(message)) return true;
  if (isAwaitingTicketLookupQuery(history) && extractBareSearchQuery(message)) return true;
  if (hasCachedTicket && isTicketInquiryFollowUp(message)) return true;
  if (hasRecentTicketInquiryContext(history) && isTicketInquiryFollowUp(message)) return true;
  if (hasRecentTicketInquiryContext(history) && extractBareSearchQuery(message)) return true;
  return false;
}

/** User wants info about an existing ticket (not create a new one). */
export function isTicketInquiryIntent(message: string): boolean {
  const m = message.trim().toLowerCase();
  if (!m) return false;

  if (
    /\b(ask|question|info|check|look up|lookup|find|search|tell me|show me|know about|consulter)\b/.test(
      m,
    ) &&
    /\btickets?\b/.test(m) &&
  ...
  if (isBareTicketTrigger(message) || /^(please\s+)?(create|open|make|créer)\b/.test(m)) {
    if (!/\b(about|existing|that|the|my|check|find|lookup|search|show|what|status|assign)\b/.test(m)) {
      if (isTicketWizardTrigger(message) && !/\b(about|check|find|what|status|assign|describe)\b/.test(m)) {
        return false;
      }
    }
  }
...
export function extractTicketIdFromText(message: string): string | undefined {
  return message.match(UUID_RE)?.[0];
}

export function extractTicketSearchQuery(
  message: string,
  history?: { role: string; content: string }[],
): string | null {
  ...
  const tryFromText = (text: string): string | null => {
    ...
    for (const p of patterns) {
      ...
      if (q && q.length >= 3 && q.length <= 200) return q;
    }
    return null;
  };
  ...
  if (followUp && history?.length) {
    ...
    if (recentTerm) return recentTerm;
...
export function extractTicketInquiryAspect(message: string): TicketInquiryAspect {
...
function assignedLabel(ticket: Ticket): string {
  if (ticket.assignedTo?.fullName) return ticket.assignedTo.fullName;
  if (ticket.assignedTo?.username) return ticket.assignedTo.username;
  if (ticket.assignedTo?.email) return ticket.assignedTo.email.split('@')[0] ?? ticket.assignedTo.email;
  if (ticket.assignedToId) return `Technician (${ticket.assignedToId.slice(0, 8)}…)`;
  return '';
}

export function formatTicketInquiryReply(
  ticket: Ticket,
  aspect: TicketInquiryAspect,
  lang: 'en' | 'fr',
): string {
  ...
  if (lang === 'fr') {
    switch (aspect) {
      ...
      default:
        return (
          `Voici le ticket « ${ticket.title} » (${shortId}…)\n` +
  ...
  switch (aspect) {
    ...
    default:
      return (
        `Here’s ticket "${ticket.title}" (${shortId}…)\n` +
...
export function formatMultipleTicketsReply(
  tickets: Ticket[],
  lang: 'en' | 'fr',
): string {
  ...
  if (lang === 'fr') {
    return (
      `J’ai trouvé plusieurs tickets :\n${lines.join('\n')}\n\n` +
      `Précisez le titre exact ou collez l’ID du ticket.`
    );
  }
  return (
    `I found several matching tickets:\n${lines.join('\n')}\n\n` +
    `Tell me the exact title or paste the ticket ID so I can answer precisely.`
  );
}

export function formatNoTicketReply(query: string, lang: 'en' | 'fr'): string {
  if (lang === 'fr') {
...
export function formatNeedQueryReply(lang: 'en' | 'fr'): string {
  const text =
    lang === 'fr'
