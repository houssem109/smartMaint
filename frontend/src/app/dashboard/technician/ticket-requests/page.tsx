...
export default function TechnicianTicketRequestsPage() {
  ...
  return (
    <ProtectedRoute allowedRoles={['technician']}>
      <Layout title="Request Tickets" showSidebar={true}>
        <div className="space-y-6">
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <h2 className="text-xl font-semibold tracking-tight">Request unassigned tickets</h2>
            <Button asChild>
              <Link href="/dashboard/technician/tickets">Go to My Tickets</Link>
            </Button>
          </div>
            ...
            <CardContent>
              {loading ? (
                ...
                <div className="flex flex-col items-center justify-center py-12 text-center text-muted-foreground">
                  <p className="mb-2">No unassigned tickets available right now.</p>
                </div>
              ) : (
                <>
                        ...
                        {paginatedTickets.map((ticket) => (
                          <TableRow key={ticket.id} className="transition-colors">
                            ...
                            <TableCell className="text-sm">
                              ...
                              ticket.assignmentRequestedById === currentUser?.id ? (
                                <Badge variant="outline">Requested by you</Badge>
                              ) : ticket.assignmentRequestStatus === 'pending' ? (
                                <Badge variant="secondary">Pending (other tech)</Badge>
                              ...
                              <div className="flex items-center justify-end gap-2">
                                {!(ticket.assignmentRequestStatus === 'pending') && (
                                  <Button
                                    variant="outline"
                                    size="sm"
                                    disabled={requestingTicketId === ticket.id}
                                    onClick={async () => {
                                      setRequestingTicketId(ticket.id);
                                      try {
                                        await api.post(`/tickets/${ticket.id}/request-self-assign`);
                                        toast.success('Request sent to admin/superadmin for approval.');
                                        await fetchTickets();
                                      } catch (err: any) {
                                  ...
                                  >
                                    {requestingTicketId === ticket.id ? 'Requesting...' : 'Request assignment'}
                                  </Button>
                                )}
