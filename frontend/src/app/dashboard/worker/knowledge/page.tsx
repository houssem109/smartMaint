  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (photoFile && res.data.id) {
        ...
        await api.post(`/knowledge/${res.data.id}/photo`, fd, {
          ...
          <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              ...
              <p className="text-sm text-muted-foreground">
                Learn from approved fixes or submit your own field observations.
              </p>
            </div>
              ...
              <CardContent className="space-y-4">
                ...
                {(selectedEntry.machineName ||
                  ...
                  <div className="flex flex-wrap gap-2">
                    {selectedEntry.machineName && (
                      <Badge variant="outline">Machine: {selectedEntry.machineName}</Badge>
                    )}
                    {selectedEntry.severity && (
                      <Badge variant="outline">Severity: {selectedEntry.severity}</Badge>
                    )}
                    {selectedEntry.symptom && (
                      <Badge variant="outline">Symptom: {selectedEntry.symptom}</Badge>
                    )}
                    {selectedEntry.rootCause && (
                      <Badge variant="outline">Root cause: {selectedEntry.rootCause}</Badge>
                    )}
                  </div>
                )}
              </CardContent>
